...
impl BlockFilter {
    ...
    /// ];
    /// let filter = BlockFilter::any()
    ///     .with_hash_any([block_hash])?;
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn with_hash_any<I, A>(mut self, hashes: I) -> anyhow::Result<Self>
    where
    ...
    /// let filter = BlockFilter::any()
    ///     .with_miner_address_any(["0xdac17f958d2ee523a2206206994597c13d831ec7"])?;
    ///
    /// // Filter by multiple miner addresses (e.g., major mining pools)
    /// let filter = BlockFilter::any()
    ///     .with_miner_address_any([
    ...
    /// ];
    /// let filter = BlockFilter::any()
    ///     .with_miner_address_any([miner_address])?;
    ///
    ...
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn with_miner_address_any<I, A>(mut self, addresses: I) -> anyhow::Result<Self>
    where
        I: IntoIterator<Item = A>,
        A: TryInto<Address>,
        A::Error: std::error::Error + Send + Sync + 'static,
