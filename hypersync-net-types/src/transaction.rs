...
impl TransactionFilter {
    ...
    /// ];
    /// let filter = TransactionFilter::any()
    ///     .with_from_address_any([sender_address])?;
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn with_from_address_any<I, A>(mut self, addresses: I) -> anyhow::Result<Self>
    where
        I: IntoIterator<Item = A>,
        A: TryInto<Address>,
        A::Error: std::error::Error + Send + Sync + 'static,
    ...
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn with_to_address_any<I, A>(mut self, addresses: I) -> anyhow::Result<Self>
    where
        I: IntoIterator<Item = A>,
        A: TryInto<Address>,
        A::Error: std::error::Error + Send + Sync + 'static,
    ...
    /// let filter = TransactionFilter::any()
    ///     .with_sighash_any([transfer_sig])?;
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn with_sighash_any<I, S>(mut self, sighashes: I) -> anyhow::Result<Self>
    where
    ...
    /// let filter = TransactionFilter::any()
    ///     .with_from_address_any(["0xdac17f958d2ee523a2206206994597c13d831ec7"])?
    ///     .with_status(1); // Only successful transactions from this address
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    ...
    /// ```
    pub fn with_type_any<I>(mut self, types: I) -> Self
    where
    ...
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn with_contract_address_any<I, A>(mut self, addresses: I) -> anyhow::Result<Self>
    where
        I: IntoIterator<Item = A>,
        A: TryInto<Address>,
        A::Error: std::error::Error + Send + Sync + 'static,
    ...
    /// ];
    /// let filter = TransactionFilter::any()
    ///     .with_hash_any([tx_hash])?;
    /// # Ok::<(), anyhow::Error>(())
    /// ```
    pub fn with_hash_any<I, H>(mut self, hashes: I) -> anyhow::Result<Self>
    where
